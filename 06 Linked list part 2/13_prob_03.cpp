...
void delete_same_neighbour_node(Node* &head, Node* &tail){
    Node* currptr=tail->prev;  // pointing to second last node
    while(currptr!=head){
        Node* prevptr=currptr->prev;
        Node* nextptr=currptr->next;
        if(prevptr->value==nextptr->value){// I need to delete current node
...
int main(){
    ...
    l1.insert_at_tail(1);
    l1.display();
    delete_same_neighbour_node(l1.head, l1.tail);
    l1.display();
}
